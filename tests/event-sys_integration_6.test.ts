...
describe("event-sys integration 6", () => {
  it("performs create-read-update cycle 6", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(600)], deployer);
  ...
  it("validates ownership in workflow 6", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(650)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
