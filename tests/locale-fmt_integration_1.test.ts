...
describe("locale-fmt integration 1", () => {
  it("performs create-read-update cycle 1", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-locale-fmt", [Cl.uint(100)], deployer);
  ...
  it("validates ownership in workflow 1", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-locale-fmt", [Cl.uint(150)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
