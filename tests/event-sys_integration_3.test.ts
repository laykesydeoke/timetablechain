...
describe("event-sys integration 3", () => {
  it("performs create-read-update cycle 3", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(300)], deployer);
  ...
  it("validates ownership in workflow 3", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(350)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
