...
describe("event-sys integration 2", () => {
  it("performs create-read-update cycle 2", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(200)], deployer);
  ...
  it("validates ownership in workflow 2", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-event-sys", [Cl.uint(250)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
