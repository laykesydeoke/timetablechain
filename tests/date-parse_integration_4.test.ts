...
describe("date-parse integration 4", () => {
  it("performs create-read-update cycle 4", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-date-parse", [Cl.uint(400)], deployer);
  ...
  it("validates ownership in workflow 4", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-date-parse", [Cl.uint(450)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
