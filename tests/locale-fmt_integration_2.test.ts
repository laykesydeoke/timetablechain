...
describe("locale-fmt integration 2", () => {
  it("performs create-read-update cycle 2", () => {
    // Create
    const cr = simnet.callPublicFn("timetablechain", "create-locale-fmt", [Cl.uint(200)], deployer);
  ...
  it("validates ownership in workflow 2", () => {
    const cr = simnet.callPublicFn("timetablechain", "create-locale-fmt", [Cl.uint(250)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
